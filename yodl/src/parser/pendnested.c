...
char *p_end_nested(register Parser *pp, HANDLER_SET_ELEMENTS newSet)
{
    ...
    pp->d_handler = (bool (**)(struct Parser *))
                    stack_tos(&pp->d_handler_st).u_voidP;
    stack_pop(&pp->d_handler_st);

    pp->d_insert =                          /* restore the inserter         */
        (void (*)(struct Parser *, char const *))
        stack_tos(&pp->d_insert_st).u_voidP;
    ...
    switch (newSet)
    {
        ...
        case DEFAULT_SET:
            ...
            pp->d_string_ptr = 
                (String *)stack_tos(&pp->d_string_st).u_voidP;
